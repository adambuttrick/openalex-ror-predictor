...
def calculate_match(row):
    ...
    valid_predictions = [(ror_id, float(score))
                         for ror_id, score in zip(predicted_ror_ids, prediction_scores)
                         if score.strip()]
    if not valid_predictions:
        return 'NP' if row['ror_id'] else 'TN'
    highest_scoring_ror_id, _ = max(valid_predictions, key=lambda x: x[1])

    if row['ror_id'] == highest_scoring_ror_id:
        return 'Y'
    elif row['ror_id'] and row['ror_id'] != highest_scoring_ror_id:
        return 'N'
    elif not row['ror_id']:
        return 'TN'
    else:
...
def calculate_counts(results_set):
    ...
    false_neg = sum(1 for row in results_set if row['match'] == 'NP' or (row['match'] == 'N' and row['ror_id']))
